...
4. Export: see "A whole block file" below for the item layout.
...
## A whole block file
For a team block file / master file / A2 file covering the topic, use **pf-blockfile**. It builds side pockets, one `A2 <argument>` hat per opposing argument, and numbered `[NU]/[DL]/[T]` responses with cards. For a single block, export it the same way: `{"hat": "A2 <their argument>"}`, then `{"card": id, "tag": "1. [T] ..."}` or `{"tag": "2. [DL] ... (analytic)"}`, with `{"text": "- warrant"}` bullets.
